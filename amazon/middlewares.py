...
from amazon.settings import PROXIES
from fake_useragent import UserAgent


class CustomizedRetryMiddleware(RetryMiddleware):
    ...
    def process_response(self, request, response, spider):
        ...
        if 'Enter the characters you see below' in response.text:
            print('需输入验证码：{}, 进行重试...'.format(request.url))
            reason = response_status_message(response.status)
            self.logger.warning('验证码, 进行重试...')
            # time.sleep(random.randint(3, 5))
            proxy_ip = requests.get('http://35.220.140.156:5010/get/').text
